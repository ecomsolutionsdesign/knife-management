...
import KnifeStatus from './KnifeStatus';          // ← NEW
...
function KnifeForm() {
    ...
    const [showKnifeStatus, setShowKnifeStatus] = useState(false);  // ← NEW
    ...
    return (
        <div className="bg-slate-100 relative">
            {/* ── Plan saving confirmation modal ─────────────────────────────── */}
            {showConfirm && (
            ...
            <div className="space-x-2 flex items-center flex-wrap gap-y-2">
                ...
                {/* ── NEW: Knife Status button ───────────────────────────────── */}
                <button
                    onClick={() => setShowKnifeStatus(true)}
                    className="bg-teal-600 hover:bg-teal-800 cursor-pointer text-slate-100 font-bold py-2 px-4 rounded-full transition-colors flex items-center gap-1.5"
                >
                    🔪 Knife Status
                </button>
            ...
            {/* ── NEW: Knife Status modal ────────────────────────────────────── */}
            {showKnifeStatus && (
                <KnifeStatus
                    currentLine={currentLine}
                    onClose={() => setShowKnifeStatus(false)}
                />
            )}
        </div>
    );
